...
export default async function JokeSlugPage({ params, searchParams }: Props) {
  ...
  return (
    <>
      ...
      <main className="bg-gray-50 top-9 text-gray-900 min-h-screen relative">
        ...
        <Link
          href={`/hi/jokes/${prevCategory.slug}`}
        ...
        <Link
          href={`/hi/jokes/${nextCategory.slug}`}
  ...
  {paginatedJokes.map((content, index) => {
    ...
    const pageUrl = `${process.env.NEXT_PUBLIC_SITE_URL}/hi/jokes/${slug}`;
        ...
        <JokeShareBar text={content} pageUrl={pageUrl} />
          ...
          {totalPages > 1 && (
            <section className="pb-12 flex justify-center gap-3 flex-wrap">
              {currentPage > 1 && (
                <Link
                  href={`/hi/jokes/${slug}?page=${currentPage - 1}`}
              ...
              {Array.from({ length: totalPages }).map((_, i) => {
                ...
                return (
                  <Link
                    key={pageNum}
                    href={`/hi/jokes/${slug}?page=${pageNum}`}
